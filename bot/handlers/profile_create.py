...
from bot.text_for_messages import (
    ...
    TEXT_PROFILE_EXP,
    TEXT_PROFILE_CANCEL, TEXT_INVALID_EMAIL
)
...
async def profile_create_firstname(
    ...
    await callback_query.message.answer(text=TEXT_PROFILE_FIRSTNAME, reply_markup=cancel_keyboard())
...
async def profile_create_lastname(
    ...
    if message.text == 'Отмена':
        await message.answer(TEXT_PROFILE_CANCEL)
        await state.clear()
        return await start(message)
    elif re.match(NAME_REG_EXP, message.text):
        await state.update_data(firstname=message.text)
        await state.set_state(ProfileCreateStates.waiting_for_lastname)
        await message.answer(text=TEXT_PROFILE_LASTNAME, reply_markup=cancel_keyboard())
    else:
...
async def profile_create_email(message: types.Message, state: FSMContext):
    ...
    if message.text == 'Отмена':
        ...
        await message.answer(text=TEXT_PROFILE_EMAIL, reply_markup=cancel_keyboard())
    else:
...
async def profile_create_professional(message: types.Message, state: FSMContext):
    ...
    if message.text == 'Отмена':
        ...
        await message.answer(text=TEXT_PROFILE_PROF_ROLE, reply_markup=cancel_keyboard())
    else:
...
async def profile_create_experience(
    ...
    await message.answer(text=TEXT_PROFILE_EXP, reply_markup=exp_type_keyboard())
